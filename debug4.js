...
// Read existing HTML quiz data
const html = fs.readFileSync('ho-chi-minh-thought-quiz.html', 'utf8');
const dataMatch = html.match(/const quizData = \[([\s\S]*?)\];\s*\/\/ END/);
let existingQuestions = [];
if (dataMatch) {
    try {
        existingQuestions = JSON.parse('[' + dataMatch[1].replace(/,\s*$/, '') + ']');
    } catch(e) {}
}
console.log('HTML has: ' + existingQuestions.length);
...
console.log('New parser unique: ' + unique.length);

// Find which ones are NOT in the existing HTML
const existingKeys = new Set(existingQuestions.map(q => normalize(q.question)));
const newOnes = unique.filter(q => !existingKeys.has(normalize(q.question)));
console.log('New questions not in HTML: ' + newOnes.length);
newOnes.forEach((q, idx) => console.log(idx + ': ' + q.question.substring(0, 80)));
